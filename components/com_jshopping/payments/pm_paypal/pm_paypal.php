...
class pm_paypal extends PaymentRoot{
	...
	function checkTransaction($pmconfigs, $order, $act){
        ...
        if (!$fp) {
            return array(0, 'Http error. Order number '.$order->order_id, $transaction, $transactiondata);
        } else {
            @fputs ($fp, $header . $req);
            while (!@feof($fp)) {
                $res = @fgets($fp, 1024);
            ...
            fclose ($fp);
            saveToLog("paymentdata.log", $debug);
			return array(0, "Error response. Order ID ".$order->order_id, $transaction, $transactiondata);
        }
